...
public class OrchestratorService {
    ...
    public void startCustomerRegistrationSaga(Customer customer) {
        log.info("SAGA INICIADO: Solicitando la creación del cliente: {}", customer.getDocument());
    ...
    public void handleCustomerCreated(String message) {
        log.info("SAGA PASO 2: El cliente fue creado con éxito. Mensaje recibido: {}", message);
        ...
        log.info("SAGA PASO 3: Solicitando creación de cuenta/login...");
    ...
    public void handleLoginCreated(String message) {
        log.info("SAGA FINALIZADA CON ÉXITO: El login fue creado. Registro completado.");
    }

    @KafkaListener(topics = "login-failed-event", groupId = "saga-orchestrator-group")
    public void handleLoginFailed(String message) {
        log.error("SAGA FALLIDA: No se pudo crear el login. Iniciando reversión (Compensación)!");
        kafkaTemplate.send("undo-customer-cmd", message);
    }
}
